...
class TaxiGpsHelper():
    ...
    dialog = None
    def run(self):
        # Get a reference to TaxiGpsBlinker, then call blink()
        # taxi_gps_blinker = App.get_running_app().marketmapview.ids.taxiblinker 
 
        # Start blinking the TaxiGpsBlinker
        # taxi_gps_blinker.blink()
    ...
    def update_blinker_position(self, *args, **kwargs):
        ...
        taxi_gps_blinker = App.get_running_app().taximapview.ids.taxiblinker 
        taxi_gps_blinker.lat = my_lat
        taxi_gps_blinker.lon = my_lon
 
        # Center map on gps
        if not self.has_centered_map:
            taximap = App.get_running_app().taximapview
            taximap.center_on(my_lat, my_lon)
